...
import type { GameStatus, Marker, PlayerRole } from "@/types/database";
import {
  ...
  type Marker as LogicMarker, type RpsState, type TwentyQuestionsState,
  type HangmanState, type WordScrambleState,
...
function maskSecret<T extends { secret: string | null; phase: string }>(
  state: T, marker: Marker | null,
): T {
  if (marker !== "X" && state.phase !== "match_over") return { ...state, secret: null };
  return state;
}
export function maskState(gameType: string, state: unknown, marker: Marker | null): unknown {
  if (gameType === RPS_GAME_TYPE) return maskRps(state as RpsState);
  if (gameType === TWENTY_QUESTIONS_GAME_TYPE) return maskSecret(state as TwentyQuestionsState, marker);
  if (gameType === HANGMAN_GAME_TYPE) return maskSecret(state as HangmanState, marker);
  if (gameType === WORD_SCRAMBLE_GAME_TYPE) return maskSecret(state as WordScrambleState, marker);
  return state;
}
...
export async function getPlayer(gameId: string, deviceToken: string) {
  const { data, error } = await supabase.from("players").select("*")
    .eq("game_id", gameId).eq("device_token", deviceToken).maybeSingle();
  if (error) throw new ApiError("not_a_player", error.message);
  return data;
}
export async function expireIfStale(game: { id: string; status: string; updated_at: number }) {
  if ((game.status === "waiting" || game.status === "in_progress") &&
      Date.now() - game.updated_at > EXPIRY_MS) {
    await supabase.from("games").update({ status: "abandoned", updated_at: Date.now() }).eq("id", game.id);
    return true;
  }
  return false;
}
...
export async function createGame(args: { gameType: string; deviceToken: string; slug?: string }) {
  ...
  if (requestedSlug) {
    ...
    if (existing) {
      if ((existing.status === "waiting" || existing.status === "in_progress") && existing.game_type === gameType) {
  ...
  const { data: game, error } = await supabase.from("games").insert({
    slug, game_type: gameType, state: freshStateFor(gameType) as object,
  ...
  const { error: pErr } = await supabase.from("players").insert({
    game_id: game.id, device_token: deviceToken, role: "initiator", marker: "X", joined_at: now,
  });
...
export async function joinGame(args: { slug: string; deviceToken: string }) {
  ...
  if (me) {
    await supabase.from("games").update({ updated_at: Date.now() }).eq("id", game.id);
    return { joined: true, me: { role: me.role as PlayerRole, marker: me.marker as Marker } };
  }
  ...
  const { error } = await supabase.from("players").insert({
    game_id: game.id, device_token: deviceToken, role: "responder", marker: "O", joined_at: joinedAt,
  });
  if (error) fail("invalid_move", error.message);
  await supabase.from("games").update({ status: "in_progress", updated_at: joinedAt }).eq("id", game.id);
  return { joined: true, me: { role: "responder" as const, marker: "O" as const } };
}
...
export async function getGameState(args: { slug: string; deviceToken: string }) {
  ...
  return {
    ...
    state: maskState(game.game_type, game.state, (me?.marker as Marker) ?? null),
    me: me ? {
      role: me.role as PlayerRole,
      marker: me.marker as Marker,
      picked: rps ? rps.picks[me.marker as LogicMarker] !== null : undefined,
    } : null,
  };
}
