 ...
 */
import { CB, PITCH } from "./constants.ts";
import type { Ball, Cap, CountersBallState, Impulse } from "./types.ts";
...
export function step(state: CountersBallState, dt: number): 0 | 1 | null {
  ...
  for (let i = 0; i < caps.length; i++) {
    for (let j = i + 1; j < caps.length; j++) {
      ...
      const r = resolveCircles(
        ...
        PITCH.capR, PITCH.capR, 3, 3, CB.E_CAP,
      );
  ...
  for (const c of caps) {
    const r = resolveCircles(
      c.x, c.y, c.vx, c.vy, ball.x, ball.y, ball.vx, ball.vy,
      PITCH.capR, PITCH.ballR, 3, 1, CB.E_BALL_CAP,
    );
    c.x = r.ax; c.y = r.ay; c.vx = r.avx; c.vy = r.avy;
    ball.x = r.bx; ball.y = r.by; ball.vx = r.bvx; ball.vy = r.bvy;
  }
